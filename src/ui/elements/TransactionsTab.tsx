...
> {
  ...
  public render() {
    ...
    const confirmedList = confirmed.map(tx => (
      <TransactionView key={tx.id} account={acc} confirmed={true} tx={tx} />
    ));
    ...
    return (
      <div className="transactionsTab">
        <div className="transactionsListDiv">
          <div className="transactionsListTop"></div>
          {unconfirmedList.length > 0 ? (
            <h3> Queue ({unconfirmedList.length}) </h3>
          ) : (
            ""
          )}
          {unconfirmedList}
          ...
          {confirmedList.length > 0 ? (
            ...
            <div className="dummyTransactionDiv">You have no transactions</div>
          )}
          {confirmedList}
        </div>
      </div>
