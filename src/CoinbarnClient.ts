...
export default class CoinbarnClient {
  ...
  public async issue(acc: Account, name: string, amount: number, decimals: number, description: string) {
    ...
    const basePayloadOuts = [new ErgoBox('', this.tokenBoxValue, height - heightDelta, sender)];
    const boxesToSpend = ErgoBox.getSolvingBoxes(myBoxes, basePayloadOuts, this.ergTransferFee);
    const token = {
      amount: amountInt,
    ...
    const payloadOutsWithTokens = [
      new ErgoBox('', this.tokenBoxValue, height - heightDelta, sender, [token], registers),
    ];
    // fill this token info to show in unconfirmed transactions
    acc.accountData.tokenInfos[token.tokenId] = payloadOutsWithTokens[0];
    const unsignedTx = Transaction.fromOutputs(boxesToSpend, payloadOutsWithTokens, this.ergTransferFee);
    const signedTx = unsignedTx.sign(acc.sk);
    return await this.explorer.broadcastTx(signedTx);
  }

  public async transfer(acc: Account, recipient: string, amount: number, tokenId: string = 'ERG') {
  ...
  private async transferInt(acc: Account, recipient: string, amountInt: number, tokenId: string = 'ERG') {
    if (tokenId === 'ERG') {
  ...
  private async ergTransfer(acc: Account, recipient: string, amountInt: number) {
    if (acc.accountData.boxes === undefined) {
      throw new Error('Balances are not synchronized yet');
    }
    const height = await this.explorer.getCurrentHeight();
    ...
    const boxesToSpend = ErgoBox.getSolvingBoxes(myBoxes, payloadOuts, this.ergTransferFee);
    const unsignedTx = Transaction.fromOutputs(boxesToSpend, payloadOuts, this.ergTransferFee);
    const signedTx = unsignedTx.sign(acc.sk);
    return await this.explorer.broadcastTx(signedTx);
  }

  private async tokenTransfer(acc: Account, recipient: string, tokenId: string, amountInt: number) {
    if (acc.accountData.boxes === undefined) {
      throw new Error('Balances are not synchronized yet');
    }
    const height = await this.explorer.getCurrentHeight();
    ...
    const payloadOuts = [new ErgoBox('', this.tokenBoxValue, height - heightDelta, new Address(recipient), tokens)];
    const myBoxes = acc.accountData.boxes;
    const boxesToSpend = ErgoBox.getSolvingBoxes(myBoxes, payloadOuts, this.tokenTransferFee);
